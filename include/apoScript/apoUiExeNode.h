...
class apoUiExenodeClosureEntry : public apoBaseExeNode
{
public:
	explicit apoUiExenodeClosureEntry(QWidget *parent, ndxml *exeNodeXml) :apoBaseExeNode()
	{
		apoBaseExeNode::m_type = EAPO_EXE_NODE_ClosureEntry;
		disableNewParam();
		disableConnectIn();
		enableOutParam();
		setNodeInfo(parent, exeNodeXml, false);		
	}
	virtual ~apoUiExenodeClosureEntry() {}
...
public:
	...
	apoBaseSlotCtrl *getInputValSlot();
	bool isGlobal();
	bool isEmptyConnect();
private:
