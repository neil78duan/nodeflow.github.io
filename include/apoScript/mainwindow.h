...
public:
	...
	void SetCompileExeCmd(const char *compileCmd) {	m_compileToExeCmd = compileCmd;}
...
protected:
	...
	void showAllView();
	bool setFileRoot(const char *project_file);
...
protected:
	...
	bool exp2cpp(const char *binData, size_t binSize);
	bool openFileView();
	...
	bool compileWithInterSetting(const char *scriptFile, std::string &outFile, int *outDebug , int *outEncodeType ,bool bWithRun = false);
	bool compileScriptNode(ndxml *node, bool bWithRun=false);
	...
	bool loadProject(ndxml *projNode);
	bool loadProject(const char *projectPaht, const char*mainFile);
	bool loadScriptFile(const char *scriptFile);
	bool _loadProjectMainfile(const char *mainScriptFile);
	...
	std::string m_compileToExeCmd;
private:
	std::string m_workingPath;
	//std::string m_netDataTypeFile;
