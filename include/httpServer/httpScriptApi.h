...
class nfHttpScriptMgr : public LogicObjectBase
{
	typedef LogicObjectBase _myBase;
public:
	nfHttpScriptMgr();
	virtual ~nfHttpScriptMgr();
	...
	typedef std::map<std::string, LogicObjectBase*, nfHttpStringLess<std::string> > objectMgr_vct;
protected:

	//objectMgr_vct m_objectMgr;
	LogicParserEngine m_logicEngine;
};
class nfHttpClientShort : public HttpConnector
{
	typedef HttpConnector _myBase;
public:
	nfHttpClientShort();
	virtual ~nfHttpClientShort();
...
class nfHttpListener : public NDHttpListener
{
	typedef NDHttpListener _myBase;
public:
	nfHttpListener(nd_fectory_base *sf = NULL);
	virtual ~nfHttpListener();
	void setPath(const char *readable, const char *writable);
	void setReadablePath(const char *readable);
	void setWritablePath(const char *writable);
...
//extern nfHttpScriptMgr * getHttpScriptObj(); //need implement this function
