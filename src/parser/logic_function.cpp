...
APOLLO_SCRIPT_API_DEF(apollo_editor_version, "editor_version()")
{
...
{
	CHECK_ARGS_NUM_ONLY(args, 1, parser);
	char md5buf[33];
	md5buf[0] = 0;
 	if (args[1].GetDataType() == OT_STRING) {
		...
		if (pSrc&& *pSrc) {
			char tmpbuf[16];
			MD5ToString((unsigned char*)MD5CryptStr16((char*)pSrc,tmpbuf),(unsigned char*) md5buf);
		}
	}
	else if (args[1].GetDataType() == OT_BINARY_DATA && args[1].GetBinarySize() >0) {
		MD5Crypt32((char*)args[1].GetBinary(), args[1].GetBinarySize(),(char*) md5buf);
	}
	if (md5buf[0] == 0) {
		parser->setErrno(NDERR_PARAM_INVALID);
		return false;
...
APOLLO_SCRIPT_API_DEF(apollo_encode_get_script, "sys_获得脚本编码()")
{
	LogicEngineRoot *root = LogicEngineRoot::get_Instant();
	const char *pName = nd_get_encode_name(root->GetEncodeType());
	if (pName) {
...
{
	LogicEngineRoot *root = LogicEngineRoot::get_Instant();
	const char *pName = nd_get_encode_name(root->getOutPutEncode());
	if (pName) {
		result.InitSet(pName);
		return true;
	}
...
APOLLO_SCRIPT_API_DEF(apollo_load_script_file, "加载脚本(script_name)")
{
...
APOLLO_SCRIPT_API_DEF(apollo_change_def_module, "改变默认模块(module_name)")
{
...
APOLLO_SCRIPT_API_DEF(apollo_load_file_as_string, "读取文件作为字符串(filename)")
{
	CHECK_ARGS_NUM(args, 2, parser);
...
APOLLO_SCRIPT_API_DEF(apollo_load_json_file, "加载json文件(jsonFilePath)")
{
	CHECK_ARGS_NUM(args, 2, parser);
...
APOLLO_SCRIPT_API_DEF(apollo_text_to_json, "文本转成JSON(json_text)")
{
	CHECK_ARGS_NUM(args, 2, parser);
...
APOLLO_SCRIPT_API_DEF(apollo_read_xml_node, "从xml读取节点(fileName, nodePath)")
{
	CHECK_ARGS_NUM(args, 3, parser);
	const char *fileName = args[1].GetText();
...
APOLLO_SCRIPT_API_DEF(apollo_tool_compile_project, "tool_compile_project(project_path)")
{
	CHECK_ARGS_NUM(args, 2, parser);
...
APOLLO_SCRIPT_API_DEF(apollo_tool_compile_file, "tool_compile_file(file,outFile,isDebug)")
{
	CHECK_ARGS_NUM(args, 3, parser);
...
APOLLO_SCRIPT_API_DEF(apollo_load_plugin, "plugin_load(str:name, str:path)")
{
	CHECK_ARGS_NUM(args, 3, parser);
	CHECK_DATA_TYPE(args[1], OT_STRING, parser);
	CHECK_DATA_TYPE(args[2], OT_STRING, parser);
	...
	LogicEngineRoot *logicEngine = LogicEngineRoot::get_Instant();
	return logicEngine->getPlugin().loadPlugin(args[1].GetText(), args[2].GetText());
}

APOLLO_SCRIPT_API_DEF(apollo_unload_plugin, "plugin_destroy(str:name)")
{
	CHECK_ARGS_NUM(args, 2, parser);
	CHECK_DATA_TYPE(args[1], OT_STRING, parser);
