...
{
	char *p = buf;
	int len = LogicEditorHelper::getClosureName(clousreNode, m_cur_funcXml, buf, bufsize);
	if (-1 == len) {
		return -1;
	}
	len += 1;	// add \0
...
// 	}
// 	else if (paramSetting->ins_type == E_INSTRUCT_TYPE_COLLOCTION) {
// 		return stepsCollect2Stream(xmlParam, p, len);
// 	}
...
#define CHECK_IS_NUMERAL(_text) do {	\
	...
	case OT_CLOSURE_NAME:
		if (!pVal || !pVal[0] || 0 == ndstricmp(pVal, "none")) {
			...
			char funcName[1024];
			ndsnprintf(funcName, sizeof(funcName), "%s@%s", m_cur_function.c_str(), pVal);
