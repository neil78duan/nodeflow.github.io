...
bool LogicParserEngine::runScript(const char *scriptName, parse_arg_list_t &args, LogicDataObj &result, int encodeType)
{
	...
	if (pcmdBuf) {
		...
		nd_logerror("script engine not found %s script-file\n", scriptName);
	}
	return false;
}
...
int LogicParserEngine::_baseCallScript(runningStack *stack)
{
	...
	//create call stack trace 
#ifdef ND_CALLSTACK_TRACE
	std::string _traceName = "[apolloScript]::";
	_traceName += stack->cmd->cmdname;
	NDCallTrace _tmp_myfunc_trace(_traceName.c_str());
#endif
	//end stack trace 
...
#define OBJ_OP_FUNC(_opfunc) do {\
		p = lp_read_stream(p, dataTypeId, m_cmdByteOrder);	\
		if (dataTypeId != OT_STRING) {						\
			m_sys_errno = LOGIC_ERR_PARSE_STRING;	break;}	\
...
bool LogicParserEngine::_getVarEx(runningStack *stack, const char *inputVarName, LogicDataObj &outVar)
{
	char name[128];
	char subName[128];
...
int LogicParserEngine::_getValue(runningStack *stack, char *pCmdStream, LogicDataObj &outValue)
{
	...
	if (OT_VARIABLE == (DBL_ELEMENT_TYPE)type){
		...
		if (ret >= 0) {
			outValue.ConvertEncode(LogicEngineRoot::get_Instant()->GetEncodeType(), ND_ENCODE_TYPE);
		}
		return ret;
	}
...
{
	if (var1.GetDataType() == OT_FLOAT || var2.GetDataType() == OT_FLOAT) {
...
bool LogicParserEngine::_call(parse_arg_list_t &args, LogicDataObj &result, const char *moduleName)
{
	...
	const func_cpp_info*  cppfunc = pRoot->getCPPFunc(funcName) ;
	if (cppfunc) {
		bool ret = false;
		if (!cppfunc->isAnsiC) {
