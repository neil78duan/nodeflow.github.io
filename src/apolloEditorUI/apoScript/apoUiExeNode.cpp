...
#undef APOUI_NODE_ID
	...
	for (int i = 0; i < number; i++) {
		if (ndstrcmp(name, exeClassname[i]) == 0) {
			return i;
		}
	}
	return 0;
...
bool apoUiExenodeNewVar::isGlobal()
{
	if (m_nodeXml) {
		const char *cmdName = ndxml_getname(m_nodeXml);
		if (cmdName) {
			if (0 == ndstricmp(cmdName, "op_global_var")) {
				return true;
			}
		}
	}
...
apoBaseSlotCtrl *apoUiExenodeNewVar::getInputValSlot()
{	
	if (m_disableNewParam && m_paramVct.size() >= 2) {
		return m_paramVct[1];
	}
	return NULL;
}
...
bool apoUiExenodeNewVar::isEmptyConnect()
{
	if (m_returnValue->getConnector()) {
		return false;
	}
	for (size_t i = 0; i < m_paramVct.size(); i++) {
		if (m_paramVct[i]->getConnector()) {
			return false;
		}
	}
	return true;
...
apoUiExenodeBool::apoUiExenodeBool(QWidget *parent, ndxml *exeNodeXml) :
	apoBaseExeNode(), m_trueSlot(NULL), m_falseSlot(NULL)
{
