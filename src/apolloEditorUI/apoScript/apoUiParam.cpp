...
bool apoBaseParam::onConnectIn(apoBaseSlotCtrl*fromSlot)
{
	...
	if (type == SLOT_FUNCTION_PARAM){
		...
		if (parentFrom->getType()== EAPO_EXE_NODE_ClosureEntry) {
			if (m_reference) {
				ndxml_setval_int(m_reference, (int)OT_CLOSURE_NAME);
			}

			char valbuf[1024];
			ndxml *fromXML = (ndxml *)parentFrom->getMyUserData();
			ndxml *xmlName = ndxml_getnode(fromXML, "closure_name");
			if (!xmlName) {
				return false;
			}
			LogicEditorHelper::buildDisplaNameValStr(ndxml_getval(xmlName), "Closure", valbuf,sizeof(valbuf));

			ndxml_setval(m_xml, valbuf);
		}
		else {
			if (m_reference) {
...
{
	if (SLOT_NODE_INPUUT_PARAM == m_slotType) {
		if (m_reference) {
