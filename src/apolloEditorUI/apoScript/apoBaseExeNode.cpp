...
{
	m_retValIsBoolCtrl = false;
	m_needTestBoolCtrl = false;
...
QString apoBaseExeNode::getTitleDisplayName()
{
	...
	if (dispName){
		const char *pRealDisp = ndstrchr(dispName, '_');
		if (pRealDisp)	{
			++pRealDisp;
			dispName = pRealDisp;
		}
		...
		if (p && *p){
			...
			ndstrncat(realDisName, varName, sizeof(realDisName));
			if (p && *p){
				++p;
				ndstrncat(realDisName, p, sizeof(realDisName));
			}
		}
...
bool apoBaseExeNode::setNodeInfo(QWidget *parent, ndxml *xmlnode, bool showParam)
{
	...
	for (int i = 0; i < ndxml_getsub_num(xmlnode); i++) {
		...
		ndxml *pCtrlXml = ndxml_getnodei(xmlnode, i);
		if (ndstricmp(ndxml_getname(pCtrlXml), "bluePrint_info") || !ndxml_getval(pCtrlXml)) {
			continue;
		}
		
		//set flag 
		if (0 == ndstricmp(ndxml_getval(pCtrlXml), "SetControlRegister")) {
			m_retValIsBoolCtrl = true;
		}
		else if (0 == ndstricmp(ndxml_getval(pCtrlXml), "TestControlRegister")) {
			m_needTestBoolCtrl = true;
		}

		else if (0 == ndstricmp(ndxml_getval(pCtrlXml), "DisableReturnVal")) {
			m_disableRetVar = true;
		}

		else if (0 == ndstricmp(ndxml_getval(pCtrlXml), "DisableToNext")) {
			m_disableToNext = true;
		}
		else if (0 == ndstricmp(ndxml_getval(pCtrlXml), "DisableNewParam")) {
			m_disableNewParam = true;
		}
		else if (0 == ndstricmp(ndxml_getval(pCtrlXml), "DisableConnectIn")) {
			m_disableIn = true;
		}
	}	
	init(titlename);
	...
	//set next anchor 
	const char *nextAnchorName = ndxml_getattr_val(m_nodeXml, "nextAnchor");
	if (nextAnchorName && *nextAnchorName) {
		ndxml *toNextAnchor = ndxml_getnode(m_nodeXml, nextAnchorName);
		if (toNextAnchor) {
	...
	const char *myTips = ndxml_getattr_val(xmlnode, "tips");
	if (myTips) {
		setTips(myTips);
	}
...
apoBaseParam* apoBaseExeNode::createParam(ndxml *xmlParam, ndxml *parent)
{
	...
	ndxml *myVal = NULL;
	ndxml *xmlName = NULL;
	apoBaseSlotCtrl::eBaseSlotType stype = apoBaseSlotCtrl::SLOT_NODE_INPUUT_PARAM;
	...
	if (paramSetting->ins_type == E_INSTRUCT_TYPE_PARAM_COLLECT){
		for (int i = 0; i < ndxml_num(xmlParam); i++) {
			...
			if (subSetting->need_type_stream == 0) {
				stype = apoBaseSlotCtrl::SLOT_VAR_NAME;
			}
		}
		parent = xmlParam;
	}
	//step 
		...
		if (paramSetting->need_type_stream == 0) {
			stype = apoBaseSlotCtrl::SLOT_VAR_NAME;
		}
	}
	else {
...
apoBaseParam* apoBaseExeNode::_param2Ctrl(ndxml *xmlParam, ndxml *parent)
{
	...
	if (pParamCtrl)	{
		pParamCtrl->setIndex(m_paramVct.size());
...
void apoBaseExeNode::setNodeAsLogicCtrl()
{
	m_retValIsBoolCtrl = true;
}
...
int apoBaseExeNode::getParamNum()
{
	return (int)m_paramVct.size();
}
apoBaseParam *apoBaseExeNode::getParam(int index) 
{
	if (m_paramVct.size() > index) {
		return m_paramVct[index];
	}
	return 0;
}
...
void apoBaseExeNode::trytoAddFunctionParam(apoBaseParam*pParamCtrl, int index)
{
	...
	if (xmlParamCollect) {
		...
		if (count>= index)	{
			...
			char buf[1024];
			ndsnprintf(buf, sizeof(buf), "<input_param name=\"Param%d\">$%d</input_param>", index, index);
	...
	pParamCtrl->setXmlDeleteNode(paramXml);
	m_paramVct.push_back(pParamCtrl);
...
void apoBaseExeNode::onAddFunctionInParam()
{
	...
	str1.ndsprintf("In%d >>",paramIndex);
...
void apoBaseExeNode::init(const QString &title)
{
    ...
    //connect in ctrl
	if (!m_disableIn)	{
		if (m_needTestBoolCtrl) {
			CREATE_CTRL_OBJECT(apoBaseSlotCtrl, "<>", cyan, m_runInNode);
		}
		else {
	...
	//return value
	if (m_retValIsBoolCtrl) {
		y = m_size.height() - E_LINE_HEIGHT + MARGIN_Y;
		CREATE_CTRL_OBJECT(apoBaseSlotCtrl, "<>", cyan, m_returnValue);
		m_returnValue->setSlotType(apoBaseSlotCtrl::SLOT_ONLY_TIPS);
		m_returnValue->setToolTip("Follow control node,without return value");
	}
	else {
		if (!m_disableRetVar) {
			y = m_size.height() - E_LINE_HEIGHT + MARGIN_Y;
