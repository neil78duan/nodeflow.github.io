...
apoXmlTreeView::apoXmlTreeView(QWidget *parent, EditCopyPaste *clipboard) : 
	dragTree(parent), m_alias(0), m_root(0), m_isInitOk(false), m_clipboard(clipboard)
{
...
void apoXmlTreeView::contextMenuEvent(QContextMenuEvent *event)
{
	...
	bool addOk = showCopyPaste(pop_menu, xml);
...
void apoXmlTreeView::onPopMenu(QAction *activeAction)
{
	...
	if (text == "del") 	{
		...
		if (!curItem || !curItem->getUserData()) {
			return;
		}
		ndxml *xml = (ndxml *)curItem->getUserData();
		m_clipboard->pushClipboard(xml, EAPO_EXE_NODE_FuncEntry);
	}
	else if (text == "Paste") {
		xmlTreeItem *curItem = (xmlTreeItem*)currentItem();
		if (!curItem || !curItem->getUserData()) {
		...
		ndxml *pasteData = m_clipboard->getClipboard();
		if (!pasteData || m_clipboard->getUserDataType() != EAPO_EXE_NODE_FuncEntry) {
			return;
		}
		do {
			int type = m_clipboard->getClipDataType(xml);
			if (type == E_INSTRUCT_TYPE_FUNC_COLLECTION || !ndxml_get_parent(xml)) {
				ndxml *myXml = ndxml_copy(pasteData);
				ndxml_insert(xml, myXml);

				QTreeWidgetItem* hNodeNew = InitTreeNode(1, myXml, curItem);
				if (hNodeNew) {
			...
			xml = ndxml_get_parent(xml);
			curItem = (xmlTreeItem*)curItem->parent();
		} while (xml);

	}
}
bool apoXmlTreeView::showCopyPaste(QMenu *pop_menu, ndxml *xml)
{
	bool ret = false;
	if (!m_clipboard) {
		return false;
	...
	//check can be copy
	int type = m_clipboard->getClipDataType(xml);
	if (type == E_INSTRUCT_TYPE_FUNCTION || type == E_INSTRUCT_TYPE_FUNC_COLLECTION) {
		QAction *copynode = new QAction("Copy", this);
		pop_menu->addAction(copynode);
		ret = true;
	}

	//check show paste
	if (m_clipboard->getClipboard()) {
		int type = m_clipboard->getClipDataType(m_clipboard->getClipboard());
		if (type == E_INSTRUCT_TYPE_FUNCTION || type == E_INSTRUCT_TYPE_FUNC_COLLECTION) {
			QAction *copynode = new QAction("Paste", this);
			pop_menu->addAction(copynode);
			ret = true;
		}
	}
