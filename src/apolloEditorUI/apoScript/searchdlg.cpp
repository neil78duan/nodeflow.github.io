...
EditCopyPaste::EditCopyPaste() : m_xmlData(0), m_userDataType(0)
{
	m_setting = apoEditorSetting::getInstant();
}

EditCopyPaste::~EditCopyPaste()
{
}

void EditCopyPaste::clearClipboard()
{
	if (m_xmlData) {
		ndxml_delxml(m_xmlData, NULL);
		m_xmlData = NULL;
	}
	m_userDataType = 0;
}

bool EditCopyPaste::pushClipboard(ndxml *data, int userDataType )
{
	const compile_setting* stepInfo = m_setting->getStepConfig(ndxml_getname(data));
	if (!stepInfo || stepInfo->ins_type == E_INSTRUCT_TYPE_COMMENT) {
		return false;
	}

	ndxml *dataNew = ndxml_copy(data);
	if (dataNew) {
		if (E_INSTRUCT_TYPE_CMD == stepInfo->ins_type) {
			ndxml_delnode(dataNew, "return_val_set_var");
		}
		clearClipboard();
		m_xmlData = dataNew;
...
ndxml *EditCopyPaste::getClipboard()
{
	return m_xmlData;
}

int EditCopyPaste::getClipDataType(ndxml *data)
{
	const compile_setting* stepInfo = m_setting->getStepConfig(ndxml_getname(data));
	if (!stepInfo ) {
		return E_INSTRUCT_TYPE_COMMENT;
	}
	return stepInfo->ins_type;
}
