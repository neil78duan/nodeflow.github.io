...
bool apoUiDetailView::_insertRow(xmlTableItem *nameItem, xmlTableItem* typeItem, xmlTableItem *valItem)
{
	...
	valItem->setToolTip(valItem->text());
...
int apoUiCommonXmlView::ShowXMLValue(ndxml *xml_node, int expand)
{
	...
	else if (ndxml_num(xml_node) ==0) {
		ShowEmptyRow(name, xml_node);
	}
...
void apoUiCommonXmlView::ShowEmptyRow(const char *name, ndxml* param)
{
	int rows = rowCount();
	setRowCount(rows + 1);
	
	xmlTableItem *tableNode = new xmlTableItem();
