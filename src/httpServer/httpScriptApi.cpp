...
nfHttpScriptMgr::nfHttpScriptMgr():m_logicEngine(this)
{

}
nfHttpScriptMgr::~nfHttpScriptMgr()
{

}

bool nfHttpScriptMgr::RunScript(parse_arg_list_t &args, LogicDataObj &result)
{
	ND_TRACE_FUNC();
...
bool nfHttpScriptMgr::RunScript(const char *script)
{
	ND_TRACE_FUNC();
...
bool nfHttpScriptMgr::SendScriptEvent(int event_id, int argc, ...)
{
	ND_TRACE_FUNC();
...
bool nfHttpScriptMgr::SendEvent0(int event)
{
	ND_TRACE_FUNC();
	return SendScriptEvent(event, 0);
}
bool nfHttpScriptMgr::SendEvent1(int event, const LogicDataObj &val1)
{
	ND_TRACE_FUNC();
...
bool nfHttpScriptMgr::opRead(const LogicDataObj& id, LogicDataObj &val)
{
...
bool nfHttpScriptMgr::opWrite(const LogicDataObj& id, const LogicDataObj &val)
{
...
bool nfHttpScriptMgr::opAdd(const LogicDataObj& id, const LogicDataObj &val)
{
...
bool nfHttpScriptMgr::opSub(const LogicDataObj& id, const  LogicDataObj &val)
{
...
LogicObjectBase *nfHttpScriptMgr::getObjectMgr(const char* destName)
{
	ND_TRACE_FUNC();
...
bool nfHttpScriptMgr::getOtherObject(const char*objName, LogicDataObj &val)
{
	ND_TRACE_FUNC();
	...
	else if (0 == ndstricmp(objName, "listener")) {
...
APOLLO_SCRIPT_API_DEF(apollo_http_error, "HTTP_error_response(connectObj, errorId, desc)")
{
	...
	nfHttpListener*pListen = dynamic_cast<nfHttpListener*>( pSession->GetParent());
	if (pListen) {
...
APOLLO_SCRIPT_API_DEF(apollo_http_file_down, "HTTP_download_file(connectObj,request,filepath)")
{
	...
	nfHttpListener *pListen = dynamic_cast<nfHttpListener*>(NDObject::FromHandle((nd_handle)val.GetObjectAddr()));
	if (!pListen) {
...
APOLLO_SCRIPT_API_DEF(apollo_http_build_body, "http_build_body( body_text)")
{
	...
	//std::string 
	do {
		const char *pVarName = ndstrchr(pInput, '$');
		if (pVarName) {
			//find var name 
			int len = 0;
			...
			ndstrncpy(pOutaddr, pInput, size);
			break;
		}
	} while (pInput && size > 0);
...
APOLLO_SCRIPT_API_DEF(apollo_http_get, "httpShort_get(host,port, path, handler,userData)")
{
	...
	nfHttpClientShort *pConn = new nfHttpClientShort;
...
static void _deattach_from_listen(nd_handle handle, void *param)
{
	...
	nfHttpClientShort *pobj = dynamic_cast<nfHttpClientShort *>( NDObject::FromHandle(handle));
	if (pobj) {
		delete pobj;
	}
}
static LogicParserEngine *_get_def_script_engine()
{
	ND_TRACE_FUNC();
	NDInstanceBase *pBase = getbase_inst();
	if (pBase && pBase->GetDeftListener()) {
		return (LogicParserEngine *)nd_message_get_script_engine(pBase->GetDeftListener()->GetHandle());
	}
	return NULL;
}

nfHttpClientShort::nfHttpClientShort()
{
...
int nfHttpClientShort::Request(NDHttpRequest &request, const char *host, int port, const char *path, const char* repHandler)
{
	ND_TRACE_FUNC();
...
void nfHttpClientShort::onResponse(NDHttpResponse *response)
{
	ND_TRACE_FUNC();
...
// 	}
	...
	LogicParserEngine *parser = _get_def_script_engine();
	if (!parser) {
		return ;
	}
	bool ret = parser->runScript(args[0].GetText(), args, result);
	if (!ret) {
		nd_logerror("run %s error %d\n", args[0].GetText(), parser->getErrno());
		return ;
	}
...
void nfHttpClientShort::OnClose()
{
	//delete this;
}
...
nfHttpListener::nfHttpListener(nd_fectory_base *sf) : _myBase(sf), m_encodeType(E_SRC_CODE_UTF_8)
{

}
nfHttpListener::~nfHttpListener()
{

}

void nfHttpListener::Destroy(int flag)
{
	ND_TRACE_FUNC();
	destroyCache();
...
void nfHttpListener::SetHttpEncode(const char *encodeName)
{
	m_encodeType = nd_get_encode_val(encodeName);
}
int nfHttpListener::getEncodeType()
{
	return m_encodeType;
}

void nfHttpListener::setPath(const char *readable, const char *writable)
{
	setReadablePath(readable);

	setWritablePath(readable);
}

void nfHttpListener::setReadablePath(const char *readable)
{
...
void nfHttpListener::setWritablePath(const char *writable)
{
	char tmp_path[ND_FILE_PATH_SIZE];
...
// 
// void *nfHttpListener::getScriptEngine()
// {
// 	return (void*)getHttpScriptObj();
// }
int nfHttpListener::onRequestScript(const char* script, NDHttpSession *session, const NDHttpRequest &request)
{
	ND_TRACE_FUNC();
	...
	LogicParserEngine *parser = _get_def_script_engine();
	if (!parser) {
		nd_logerror("can not get script parser\n");
		return -1;
	}
	bool ret = parser->runScript(script, args, result);
	if (!ret) {
		nd_logerror("run %s error %d\n", script, parser->getErrno());
		return -1;
	}
	return 0;
}

void nfHttpListener::destroyCache()
{
	ND_TRACE_FUNC();
...
bool nfHttpListener::cacheFile(const char *filePath)
{
	ND_TRACE_FUNC();
	size_t size = 0;
...
bool nfHttpListener::uncacheFile(const char*filePath)
{
	ND_TRACE_FUNC();
...
bool nfHttpListener::downloadFile(const char *filePath, NDHttpSession *session, const NDHttpRequest &request)
{
	ND_TRACE_FUNC();
	NDINT64 offset = 0;
	NDINT64 size = 0;
	...
	if (pRange) {
		char tmp[128];
		ndsnprintf(tmp, sizeof(tmp), "bytes %lld-%lld/%lld", offset, (offset + size), totalSize);
	...
	response.addHeader("Content-Type", nfHttpListener::getFileType(filePath));

	//get file name 
	std::string fname = NDHttpParser::textToURLcode(nd_filename(filePath));
	if (fname.size()) {
		ndsnprintf(tmp_path, sizeof(tmp_path), "attachment;filename=%s", fname.c_str());
...
bool nfHttpListener::getFileOffset(const char*pRange, NDINT64 &size, NDINT64 &offset)
{
	ND_TRACE_FUNC();
...
void nfHttpListener::releaseFile(void *fileData)
{
	ND_TRACE_FUNC();
	for (fileCache_map_t::iterator it = m_fileCache.begin(); it != m_fileCache.end(); ++it) {
...
void *nfHttpListener::loadFile(const char *filePath, NDINT64 offset, NDINT64 &size, size_t &fileSize)
{
	ND_TRACE_FUNC();
	fileCache_map_t::iterator it = m_fileCache.find(filePath);
	if (it == m_fileCache.end()) {
...
void *nfHttpListener::_loadFile(const char *filePath, NDINT64 offset, NDINT64 &size, size_t &fileSize)
{
	ND_TRACE_FUNC();
...
const char *nfHttpListener::getFileType(const char *fileName)
{
