...
// 	mysql_options(&m_mysqlinst, MYSQL_OPT_CONNECT_TIMEOUT,(const void*) &oflag) ;
	...
	ndstrncpy(m_host, host, sizeof(m_host)) ;
	ndstrncpy(m_database, dbinstance, sizeof(m_database)) ;
	ndstrncpy(m_user, user, sizeof(m_user)) ;
	ndstrncpy(m_password, passwd, sizeof(m_password)) ;
	if(-1==on_connect() ) {
...
{ 
	char sql_smt[1024];
	ndsnprintf(sql_smt,1024,"lock table %s  %s ", tablename, (flag) ? "write" : "read" );
